...
On crée différents bag of words (\textit{bog}) pour nos documents afin d'en tester la pertinence.
%------------------------------------------------------------------------------------------
\subsubsection{Pré-processing}

On effectue en amont de la construction du \textit{bog} un nettoyage du texte. Les options retenues sont :
\begin{itemize}
...
On supprime les mots blancs avant construction du \textit{bog}. La liste des stopwords est obtenue par concaténation des stopwords français donnés par la bibliothèque \texttt{nltk} auxquels on ajoute une liste personnelle, établie à partir de la liste des mots les plus présents et non-discriminants.
...
On note que, sans surprise, les auxiliaires \textbf{être} et \textbf{avoir} arrivent en tête des mots les plus fréquents. Il convient donc de les ajouter aux mots blancs.
...
On conjugue bigrammes et racinisation. Taille du vocabulaire =  345610
\begin{lstlisting}
ngram_range = (1,2) 
...
On note que les mots les plus fréquents sont des monogrammes.
%------------------------------------------------------------------------------------------
...
D'autre part, il m'a semblé pertinent de ré-équilibrer les données. J'ai donc créé un neuvième \textit{bog} en ramenant le nombre d'interventions de M.Chirac à un nombre égal à celui de M.Mitterrand. 
...
On va appliquer trois méthodes de classification sur les différents \textit{bog} :
\begin{itemize}
\item Naïve Bayes
...
X_train = BOW9
y_train = new_labels #alllabs si BOG != BOG9 (plus petit)

#Choix du modèle
model = LogisticRegression(random_state=0, solver='liblinear', max_iter=100, tol=1e-8, C=100.0)

# Entrainement le modèle
model.fit(X_train, y_train)

#appliquer le même BOW sur les phrases mystère
...
df = pd.DataFrame({
    ...
    "probabilite_chirac": probabilites
})
